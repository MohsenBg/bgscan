...
func TestWriterConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mutateCfg:     func(c *config.WriterConfig) { c.MergeFlushInterval = config.NewDurationMS(50 * time.Millisecond) },
			wantErrKeys:   []string{"MergeFlushInterval"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.WriterConfig) {
				if c.MergeFlushInterval.Duration() != def.MergeFlushInterval.Duration() {
					t.Errorf("MergeFlushInterval = %v, want %v", c.MergeFlushInterval.Duration(), def.MergeFlushInterval.Duration())
				}
			},
		},
		{
			name:          "ChanSize too high",
			mutateCfg:     func(c *config.WriterConfig) { c.ChanSize = 1_000_001 },
			wantErrKeys:   []string{"ChanSize"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.WriterConfig) {
				if c.ChanSize != def.ChanSize {
					t.Errorf("ChanSize = %d, want %d", c.ChanSize, def.ChanSize)
				}
			},
		},
		{
			name:          "BatchSize too low",
			mutateCfg:     func(c *config.WriterConfig) { c.BatchSize = 0 },
			wantErrKeys:   []string{"BatchSize"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.WriterConfig) {
				if c.BatchSize != def.BatchSize {
					t.Errorf("BatchSize = %d, want %d", c.BatchSize, def.BatchSize)
				}
			},
		},
		{
			name:          "ResultBaseDir invalid",
			mutateCfg:     func(c *config.WriterConfig) { c.ResultBaseDir = "invalid\\dir?" },
			wantErrKeys:   []string{"ResultBaseDir"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.WriterConfig) {
				if c.ResultBaseDir != def.ResultBaseDir {
					t.Errorf("ResultBaseDir = %q, want %q", c.ResultBaseDir, def.ResultBaseDir)
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, key := range tt.wantErrKeys {
				if _, ok := errs[key]; !ok {
					t.Errorf("ValidateWriter() missing expected error for key %q", key)
				}
			}
