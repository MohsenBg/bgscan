...
func parseTrojan(link string) (*ParseResult, error) {
	...
	ob := Outbound{
		...
		"settings": map[string]any{
			"servers": []any{
				map[string]any{"address": addressPlaceholder, "port": port, "password": pw},
			},
		},
		"streamSettings": stream,
	}
...
func parseShadowsocks(link string) (*ParseResult, error) {
	...
	if at >= 0 {
		...
		ob := Outbound{
			...
			"settings": map[string]any{
				"servers": []any{
					map[string]any{"address": addressPlaceholder, "port": port, "password": pass, "method": method},
				},
			},
		}
		return &ParseResult{Outbound: ob, Identity: identity}, nil
...
func parseHysteria2(link string) (*ParseResult, error) {
	...
	ob := Outbound{
		"protocol":       "hysteria",
		"tag":            decodeHash(u.Fragment),
		"settings":       map[string]any{"address": addressPlaceholder, "port": port, "version": 2},
		"streamSettings": stream,
	}
	return &ParseResult{Outbound: ob, Identity: identity}, nil
}
...
func buildStream(network, security string) map[string]any {
	...
	case "xhttp":
		stream["xhttpSettings"] = map[string]any{
			"path": "/", "host": "", "mode": "auto",
			"extra": map[string]any{"headers": map[string]any{}, "xPaddingBytes": "100-1000", "scMaxEachPostBytes": "1000000"},
		}
	default:
		stream["tcpSettings"] = map[string]any{"header": map[string]any{"type": "none"}}
	}
