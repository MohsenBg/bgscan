...
| --------- | --------- | ------------- |
| `timeout` | platform-dependent | Maximum duration (in milliseconds) to wait for the Xray connectivity test to complete. |
| `speed_test_timeout` | platform-dependent | Maximum duration (in milliseconds) to wait for each download/upload speed test. |
| `workers` | platform-dependent | Number of concurrent workers performing Xray connectivity tests. |
...
| `download_speed` | platform-dependent | Minimum acceptable download speed in Kbps (used when the test type includes download). |
| `upload_speed` | platform-dependent | Minimum acceptable upload speed in Kbps (used when the test type includes upload). |
...
| `max_concurrent_dials` | platform-dependent | Max concurrent outbound dials process-wide (`0` = unlimited). |
| `dial_max_attempts` | platform-dependent | Dial attempts per outbound dispatch. |
...
Caps how many outbound dials may run at the same time across all workers. `0` means unlimited. Lower it when the machine runs out of sockets or file descriptors during large scans. Range is 0-10000.
...
How many times each outbound dispatch retries its dial before giving up. Range is 1-10.
