...
- **کارکرد**: برای هر هدف یک نمونهٔ Xray داخل خود برنامه با Outbound Template انتخابی بالا می‌آورد و زمان اتصال را از طریق Proxy محلی اندازه‌گیری می‌کند. بسته به `connectivity_test_type`، تست دانلود، آپلود یا هر دو هم اجرا می‌شود و هدفی که سرعتش از حداقل تعیین‌شده کمتر باشد ناموفق حساب می‌شود.
