...
const (
	...
	descDownloadSpeed        = "Target download data size (in KB) to test against within the configured timeout. Used when connectivity test type includes download."
	descUploadSpeed          = "Target upload data size (in KB) to test against within the configured timeout. Used when connectivity test type includes upload."
...
func New(l *layout.Layout, name string) *Model {
	...
	downloadSpeed := intInput(l, "Enter download speed (kb)", cfg.DownloadSpeed,
		func(v string) error {
			n, err := strconv.Atoi(v)
			if err != nil {
				return err
	...
	uploadSpeed := intInput(l, "Enter upload speed (kb)", cfg.UploadSpeed,
		func(v string) error {
			n, err := strconv.Atoi(v)
			if err != nil {
				return err
	...
	fields := []inspector.Field{
		...
		{Name: "Download Speed (kb)", Description: descDownloadSpeed, Group: groupXray, Input: inspector.Adapt(downloadSpeed), Visible: includesDownload, Format: inspector.FormatInt},
		{Name: "Upload Speed (kb)", Description: descUploadSpeed, Group: groupXray, Input: inspector.Adapt(uploadSpeed), Visible: includesUpload, Format: inspector.FormatInt},
