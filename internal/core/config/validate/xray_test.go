...
func TestXrayConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mutateCfg:     func(c *config.XrayConfig) { c.Workers = 1001 },
		...
		},
		{
			...
			mutateCfg:     func(c *config.XrayConfig) { c.ConnectivityTestType = 99 },
			wantErrKeys:   []string{"ConnectivityTestType"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.XrayConfig) {
				if c.ConnectivityTestType != def.ConnectivityTestType {
					t.Errorf("ConnectivityTestType = %d, want %d", c.ConnectivityTestType, def.ConnectivityTestType)
				}
			},
		},
		{
		...
		{
			name:          "UploadSpeed negative",
			mutateCfg:     func(c *config.XrayConfig) { c.UploadSpeed = -1 },
		...
		},
		{
			...
			mutateCfg:     func(c *config.XrayConfig) { c.Timeout = config.NewDurationMS(61 * time.Second) },
		...
		},
		{
			...
			mutateCfg:     func(c *config.XrayConfig) { c.PreScanType = "invalid" },
			wantErrKeys:   []string{"PreScanType"},
			wantWarnCount: 1,
			checkFixed: func(t *testing.T, c *config.XrayConfig) {
				if c.PreScanType != def.PreScanType {
					t.Errorf("PreScanType = %q, want %q", c.PreScanType, def.PreScanType)
				}
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, key := range tt.wantErrKeys {
				if _, ok := errs[key]; !ok {
					t.Errorf("ValidateXray() missing expected error for key %q", key)
				}
			}
